...
from rsa_digsig import rsa_client
...
client_digsig = rsa_client(message)     #returns bytes signature

# concatenating message with a delimiter "+" for server-side handling of msg and sig
message += "+"
message_bytes = message.encode()
message_bytes += client_digsig    # now message has message, "+", and digsig

# padded message into 16 bytes just incase user does not input 16 bytes
...
print("Cipher text:", cipherText) #still outputs to user
# print("Signature length:", len(signed_msg))
...
# cliSock.sendall(list_of_enc_digsig) cannot send object lis as socket sends in bytes
